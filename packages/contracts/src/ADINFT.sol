...
/**
 * @title ADINFT
 * @notice ERC-721 NFT template for ADI Chain.
 *         ADI's low fees make NFT minting practical at scale.
 *
 * @dev Minimal ERC-721 without OpenZeppelin to avoid version pinning.
 *      For production, consider: import "@openzeppelin/contracts/token/ERC721/ERC721.sol";
 *
 * @example
 *   const nft = await deployContract("ADINFT", ["My Collection", "MYC", "https://api.example.com/metadata/"]);
 *   await nft.mint(userAddress); // free mint
 *   await nft.mintTo(userAddress, 5); // owner mint to specific address
 */
contract ADINFT {
    string public name;
    ...
    /// @notice Public mint — caller receives the NFT. Payable if mintPrice > 0.
    ...
    /// @notice Owner can mint to any address for free (airdrop / reserve).
